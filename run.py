...
def handle_user_choice():
    """
    Prompt the user for a choice between continuing, returning to the
    main menu, or exiting the program.
    This function continuously prompts the user for a choice until a
    ...
    while True:
        ...
        if choice == 'c':
            ...
            print(Fore.RED + "\nInvalid choice. Please press 'c' to continue,"
                  "'m' to return to the main menu, or 'q' to exit the program")
            print(Style.RESET_ALL)
...
def program1_convert_numbers_to_words():
    ...
    Parameters:
    - number (int): The integer number to be converted into words.
    ...
    while True:
        def number_to_words(number):
            ...
            if 0 <= number < 20:
                ...
                return (
                    word_to_num[number // 10 * 10] +
                    ('' if number % 10 == 0 else ' ' +
                        word_to_num[number % 10])
                )
            elif 100 <= number < 1000:
                return (
                    word_to_num[number // 100] + ' hundred' +
                    (' and ' + number_to_words(number % 100)
                        if number % 100 != 0 else '')
                )
            elif 1000 <= number < 1000000:
                return (
                    number_to_words(number // 1000) + ' thousand' +
                    (' ' + number_to_words(number % 1000)
                        if number % 1000 != 0 else '')
                )
            elif 1000000 <= number < 1000000000:
                return (
                    number_to_words(number // 1000000) + ' million' +
                    (' ' + number_to_words(number % 1000000)
                        if number % 1000000 != 0 else '')
                )
            elif 1000000000 <= number < 1000000000000:
                return (
                    number_to_words(number // 1000000000) + ' billion' +
                    (' ' + number_to_words(number % 1000000000)
                        if number % 1000000000 != 0 else '')
                )
            elif 1000000000000 <= number < 1000000000000000:
                return (
                    number_to_words(number // 1000000000000) + ' trillion' +
                    (' ' + number_to_words(number % 1000000000000)
...
def program2_get_weather():
    ...
    while True:
        ...
        if not city.strip() or not city.isalnum():
        ...
        if weather_data.get('cod') == '404':
            ...
            timezone_offset = datetime.timedelta(
...
def program3_get_day_of_birth():
    ...
    their date of birth.
    Parameters:
    - date_str (str): A string representing the date of birth
    in the format 'DD-MM-YYYY'.
    Returns:
    - str: The day of the week on which the person was born
    (e.g., 'Monday', 'Tuesday', etc.).
    Raises:
    - ValueError: If the input date string is not in
    the correct format or if it represents an invalid date.
    """
    while True:
        try:
...
def program4_count_all_characters():
    ...
    Parameters:
    - input_text(str): The input text for which character counts
    are to be calculated.
    Returns:
    - dict: A dictionary containing the count of occurrences of each character
    in the input text.
    """
    ...
    while True:
        ...
        # Count characters
        alphabet_count, digit_count, special_char_count, space_count = \
...
def program5_guess_a_number():
    ...
    Returns:
    - str: A message indicating whether the user guessed
    ...
    while True:
        try:
            def guess_a_number(lower_limit, upper_limit):
                ...
                while True:
                    guess = int(input(f"\nGuess a number between {lower_limit}"
                                      f" and {upper_limit}: "))
                    ...
                    if guess < lower_limit or guess > upper_limit:
                        print(Fore.RED + "\nNumber is out of range. "
                                         "Please guess a number within the "
                                         "given range 1 to 10")
                        print(Style.RESET_ALL)
                    elif guess == target_number:
                        print(Fore.GREEN + f"\nCongratulations! You guessed "
                                           f"the number {target_number} "
                                           f"correctly in {attempts}"
                                           f" attempts.")
                        print(Style.RESET_ALL)
                        break
                    elif guess < target_number:
                        ...
                        print(Fore.BLUE + "\nEntered number is high. Try again"
                                          " with a lower number.")
                        print(Style.RESET_ALL)
...
def main():
    ...
    while True:
        program_choice = input(Fore.CYAN + "Please choose a program and type a"
                                           "number between (1-5): ")
        print(Style.RESET_ALL)
        ...
        if program_choice == '1':
            ...
            print(Fore.RED + "\nInvalid choice. Please choose a program"
                             " between 1 to 5.")
            print(Style.RESET_ALL)
